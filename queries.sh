...
echo "$($PSQL "SELECT DISTINCT name FROM (SELECT t1.name FROM games g JOIN teams t1 ON g.winner_id = t1.team_id WHERE g.year = 2014 AND g.round = 'Eighth-Final' UNION SELECT t2.name FROM games g JOIN teams t2 ON g.opponent_id = t2.team_id WHERE g.year = 2014 AND g.round = 'Eighth-Final') AS all_teams ORDER BY name;")"
